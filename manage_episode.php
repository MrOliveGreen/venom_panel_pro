...
if(isset($_POST['delete_id']))
{
  ...
  {
    $flag = 2;
    $alert = 'Episode Operation Failed!';
  }
}
...
if(isset($_POST['pause_id']))
{
  ...
  $result = $get->change_episode_status($id, 2);
  if($result)
  {
    $flag = 1;
    $alert = 'Episode Paused Successfully!';
  }
  else
  {
    $flag = 2;
...
if(isset($_POST['play_id']))
{
  ...
  $result = $get->change_episode_status($id, 1);
  if($result)
  {
    $flag = 1;
    $alert = 'Episode Allowed Successfully!';
  }
  else
  {
    $flag = 2;
...
if(isset($_POST['download_id']))
{
  ...
  $result = $get->change_episode_status($id, 3);
  if($result)
  {
    $flag = 1;
    $alert = 'Episode Downloading Now!';
  }
  else
  {
    $flag = 2;
...
if(isset($_POST['transcode_id']))
{
  $id = base64_decode($_POST['transcode_id']);

  $result = $get->change_episode_status($id, 4);
  if($result)
  {
    $flag = 1;
    $alert = 'Episode Transcoding Now!';
  }
  else
  {
    $flag = 2;
      ...
      <div class="col-md-12"> 
        ...
        <div class="panel panel-default">
          <div class="panel-heading">
            <h3 class="panel-title">Manage Episodes</h3>
          </div>
...
$(document).ready(function() {
    $('#table').dataTable( {
      ...
      "bProcessing": true,
      "bServerSide": true,
      "sAjaxSource": "common//episode_processing.php",
      "aoColumns":[
          {"bSortable": true},
          {"bSortable": true},
          {"bSortable": true},
