<?php 
include 'head.php';
if(isset($_SESSION['user_info']) && isset($_GET['user_id'])){
...
if($connection==1){
  ...
  $connection = $insert->edit_reseller_admin($user_id, $_POST['name'],$pass,$_POST['email'],isset($_POST['dns']) ? $_POST['dns'] : '', isset($_POST['note']) ? $_POST['note'] : '' ,isset($_POST['packages']) ? $_POST['packages'] : '', isset($_POST['custom_pkg']) ? $_POST['custom_pkg'] : '', isset($_POST['bouquets']) ? $_POST['bouquets'] : '', isset($_POST['credit']) ? $_POST['credit'] : 0, $_POST['user_type']);
          ...
          <div class="panel panel-default">
            ...
            <div class="panel-body">
              ...
              <div class="form-group">
                ...
                <div class="col-md-6 col-xs-12">
                  <div class="input-group"> <span class="input-group-addon"><span class="fa fa-pencil"></span></span>
                    <input type="text" name="credit" class="form-control" placeholder="0" value = "<?php echo $user['user_credit']; ?>"/>
                  </div>
              ...
              <div class="form-group">
                ...
                <div class="col-md-6 col-xs-12">
                  <div class="input-group"> <span class="input-group-addon"><span class="fa fa-pencil"></span></span>
                    <input type="text" name="dns" class="form-control" value = "<?php echo $user['user_stream_dns']; ?>"/>
                  </div>
              ...
              <div class="form-group">
                ...
                <div class="col-md-6 col-xs-12">
                  <div class="input-group"> <span class="input-group-addon"><span class="fa fa-pencil"></span></span>
                    <input type="text" name="note" class="form-control" value = "<?php echo $user['user_note']; ?>"/>
                  </div>
              ...
              <div class="form-group">
                ...
                <div class="col-md-6 col-xs-12">
                    ...
                    <select name = "user_type" style = "margin-right:20px; border-radius: 4px; height:30px; width:200px;">
                      ...
                      <option value = "1" <?php echo ($user['user_owner_id'] == 0 ? "selected" : "");?>> Reseller </option>
                      <option value = "0" <?php echo ($user['user_is_admin'] == 1 ? "selected" : "");?>> Admin </option>
                      <option value = "2" <?php echo ($user['user_owner_id'] != 0 ? "selected" : "");?>> SubReseller </option>
                      ...
                      <div class="input-group">
                          ...
                          <select name = "packages[ ]" class="multiselect" multiple='multiple'>
                            ...
                            if($user['user_package_id'] == '')
                              ...
                              $user_pkg = json_decode($user['user_package_id']);

                              while($package = mysqli_fetch_assoc($packages))
                              {
                                  echo "<option value='".$package['package_id']."'".(in_array($package['package_id'], $user_pkg)? "selected" : "").">".$package['package_name']."</option>";
                              }
                            ?>
                          </select>
                      </div>                                            
                          ...
                          <div class = "row" style = "margin-top:20px;">
                            <div class = "col-md-5">
                              <select class="custom-select"  style = "width:250px;  height:180px; font-size: 16px;" id = "custom_box" multiple>
                                <?php 
                                $custom = json_decode($user['user_custom_package']);
                                $names = array();$types = array();$durations = array();$credits = array();
                                for($i = 0; $i < count($custom); $i ++)
                                {
                                  echo '<option>'.$custom[$i][0].'</option>';
                                  $names[] = $custom[$i][0];
                                  $types[] = $custom[$i][1];
                                  $durations[] = $custom[$i][2];
                                  $credits[] = $custom[$i][3];
                                }  
                                ?>
                              </select>
                              <input id = "org_name" value = '<?php if(count($names)) echo json_encode($names);?>' hidden/>
                              <input id = "org_type" value = '<?php if(count($types)) echo json_encode($types);?>' hidden/>
                              <input id = "org_duration" value = '<?php if(count($durations)) echo json_encode($durations);?>' hidden/>
                              <input id = "org_credit" value = '<?php if(count($credits)) echo json_encode($credits);?>' hidden/>
                          </div>
                            <div class = "col-md-5">
                  ...
                  <div class="col-md-6 col-xs-12">                                            
                      <div class="input-group">
                          ...
                          <select name = "bouquets[ ]" class="multiselect" multiple='multiple'>
                            ...
                            if($user['user_bouquet_id'] == '')
                              $user_bouquet = array();
                            else
                              $user_bouquet = json_decode($user['user_bouquet_id']);

                              while($bouquet = mysqli_fetch_assoc($bouquets))
                              {
                                    echo "<option value='".$bouquet['bouquet_id']."'".(in_array($bouquet['bouquet_id'], $user_bouquet)? "selected" : "").">".$bouquet['bouquet_name']."</option>";
                              }
                            ?>
                          </select>
                      </div>                                            
...
<script>
  ...
  $(document).ready(function(){
    ...
    if($("#org_name")[0].value != '')
    {
      pkgname = JSON.parse($("#org_name")[0].value);
      type = JSON.parse($("#org_type")[0].value);
      duration = JSON.parse($("#org_duration")[0].value);
      credit = JSON.parse($("#org_credit")[0].value);
