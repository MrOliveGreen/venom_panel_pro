      ...
      <div class="col-md-12"> 
        ...
        <div class="panel panel-default">
          <div class="panel-heading">
            <h3 class="panel-title">Manage Bouquets</h3>
          </div>
          <div class="panel-body">
            <table class="display" id = "table" style = "width:100%">
              <thead>
                <tr>
                  <th>BOUQUET NAME</th>
                  <th>BOUQUET STREAMS COUNT</th>
                  <th>BOUQUET MOVIES COUNT</th>
                  <th>BOUQUET SERIES COUNT</th>
                  <th>ACTION</th>
                </tr>
              </thead>
              <tbody>
                <?php 
				        while($bouquet = mysqli_fetch_assoc($bouquet_obj)){
				        ?>
                <tr>
                  <td style="cursor:pointer;"><?php echo $bouquet['bouquet_name']; ?></td>
                  <td style="cursor:pointer;">
                    <?php 
                      $streams = $bouquet['bouquet_streams'];
                      if($streams != '')
                        echo count(json_decode($bouquet['bouquet_streams']));
                      else
                        echo '0';
                    ?>
                  </td>
                  <td style="cursor:pointer;">
                    <?php 
                      $movies = $bouquet['bouquet_movies'];
                      if($movies != '')
                        echo count(json_decode($bouquet['bouquet_movies']));
                      else
                        echo '0';
                    ?>
                  </td>
                  <td style="cursor:pointer;">
                    <?php 
                      $series = $bouquet['bouquet_series'];
                      if($series != '')
                        echo count(json_decode($bouquet['bouquet_series']));
                      else
                        echo '0';
                    ?>
                  </td>
                  <td>
                    <button style = "float:left; position:relative;" class="btn btn-default btn-rounded btn-sm"><a href = "edit_bouquet.php?bouquet_id=<?php echo base64_encode($bouquet['bouquet_id']);?>"><span class="fa fa-pencil"></span></button>
                    <form style = "float:left; position:relative;" method = "post" enctype="multipart/form-data">
                      <input type = "text" name="delete_id" value="<?php echo base64_encode($bouquet['bouquet_id']);?>" hidden>
